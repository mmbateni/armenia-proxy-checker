# ─────────────────────────────────────────────────────────────────────────────
# Armenia Proxy Tester — Live-Verified Edition
...
fetch_live_verified <- function() {
  ...
  # ── proxyscrape API — returns only currently-working AM proxies ─────────────
  for (proto in c("http","socks4","socks5")) {
    url <- paste0(
      "https://api.proxyscrape.com/v3/free-proxy-list/get",
      "?request=displayproxies&country=am&protocol=", proto,
  ...
  # ── proxifly AM-specific list (updated every 5 min) ─────────────────────────
  tryCatch({
    ...
    if (length(m)) {
      log_ts(sprintf("  proxifly AM          → %d proxies", length(m)))
      proxies <- c(proxies, m)
    }
  }, error=function(e) log_ts(sprintf("  ! proxifly: %s", conditionMessage(e))))

  # ── Geonode AM — sorted by lastChecked ──────────────────────────────────────
  tryCatch({
    url <- paste0("https://proxylist.geonode.com/api/proxy-list",
                  "?country=AM&limit=100&page=1&sort_by=lastChecked&sort_type=desc")
    r   <- request(url) |> req_timeout(15) |> req_headers("User-Agent"=UA) |> req_perform()
    d   <- resp_body_json(r)
    ...
    if (length(geo)) {
      log_ts(sprintf("  Geonode AM           → %d proxies", length(geo)))
  ...
  for (proto in c("socks5","http")) {
    tryCatch({
      url <- paste0("https://proxydb.net/?protocol=", proto, "&country=AM")
      r   <- request(url) |> req_timeout(15) |> req_headers("User-Agent"=UA) |> req_perform()
      m   <- regmatches(resp_body_string(r),
                        gregexpr("\\d{1,3}(?:\\.\\d{1,3}){3}:\\d{2,5}",
                                 resp_body_string(r), perl=TRUE))[[1]]
      if (length(m)) {
        log_ts(sprintf("  proxydb AM %-7s   → %d proxies", proto, length(m)))
        proxies <- c(proxies, m)
      }
...
test_one <- function(proxy_str, tcp_to, http_to, verify_url, fallback_urls, ua) {
  ...
  # Try protocols — SOCKS5 first (most common on AM lists)
    ...
    result <- tryCatch({
      ...
      if (res$status_code>=200 && res$status_code<400) {
        ...
        if (!is.null(d$status) && d$status=="success")
          list(proxy=proxy_str, status=if(cc=="AM")"WORKING" else "WRONG_COUNTRY",
...
cli::cli_rule(center=" Armenia Proxy Tester — Live-Verified Edition ")
cat("\n")
...
proxies <- character(0)
if (!args$`skip-fetch`) {
  log_ts("Fetching proxies verified alive RIGHT NOW (sources with <5min update cycle)…")
  proxies <- fetch_live_verified()
  log_ts(sprintf("Live-verified pool: %d unique AM proxies", length(proxies)))
}
...
cli::cli_rule()
cat(sprintf("  \033[32m%2d\033[0m Working   — Armenian exit IP confirmed\n",  length(working)))
...
if (length(working)==0 && length(unverif)==0) {
  ...
  cli::cli_bullets(c(
    "i"="Free Armenian proxies are short-lived — try again in a few hours.",
    "i"="Run again when fresh proxies appear in the upstream sources.",
